...
export default function SimpleContactCard({ 
  ...
  const maskPhone = (phoneValue: string) => {
    return `${phoneValue.substring(0, 7)}...`;
  };

  const displayEmail = revealedEmail ? email : maskEmail(email);
  const displayPhone = revealedPhone ? phone : maskPhone(phone);

  return (
    <div className="flex flex-col gap-4 p-6 bg-neutral-900/30 border border-neutral-800 rounded-4xl hover:border-neutral-700 transition-colors duration-300">
      <div>
      ...
      <div className="flex flex-col gap-3 mt-2">
        ...
        <div className="flex items-center gap-2 text-sm">
          <span className="text-neutral-500 min-w-[70px]">{emailLabel}:</span>
          {revealedEmail ? (
            <a 
              href={`mailto:${email}`}
        ...
        {/* Phone */}
        <div className="flex items-center gap-2 text-sm">
          <span className="text-neutral-500 min-w-[70px]">{phoneLabel}:</span>
          {revealedPhone ? (
            <a 
              href={`tel:${phone}`}
              className="text-white hover:text-blue-400 transition-colors"
        ...
        {/* Discord */}
        <div className="flex items-center gap-2 text-sm">
          <span className="text-neutral-500 min-w-[70px]">{discordLabel}:</span>
          <a 
            href={discord}
            target="_blank"
            rel="noopener noreferrer"
            className="text-white hover:text-[#5865F2] transition-colors flex items-center gap-1"
          >
